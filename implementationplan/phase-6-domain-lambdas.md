...
stories:
  ...
  - id: 6.4
    ...
    acceptance_criteria:
      ...
      - Integration test: A (Male) and B (Female) are friends (set up via 6.2 friend-request flow which is also deployed). A POST /v1/blocks {userId: B} → assert (i) friendships row gone, (ii) any pending friend_request between them gone, (iii) ChatRooms.status — if the test pre-creates a row via direct boto3 PutItem before POSTing the block, the row should be 'deactivated' with deactivated_by=A. If no chat room is pre-created, the conditional update no-ops and the request still returns 200. Then A POST /v1/friend-requests {toUserId: B} returns 409 BLOCKED. A DELETE /v1/blocks/B succeeds, the chat-room row (if any) reactivates, and a follow-up POST /v1/friend-requests now succeeds.

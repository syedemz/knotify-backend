"""
knotify-anonymize-chat-messages Lambda handler — story 9.6

Anonymizes all ChatMessages rows sent by the deleted user across every room
they were a member of.  Called from the account-deletion Step Functions state
machine inside the ParallelCleanup block, after DeactivateChatRooms has already
deleted the user's ChatRoomMembership rows.
...
For each room_id, the Lambda:
  ...
  2. Calls UpdateItem on each matching row to set sender_id='[deleted-user]'
     while leaving content and all other attributes untouched.
...
linear in the number of messages the user sent in the rooms they were a member
of, not in total chat traffic.

Continuation token contract:
  Input fields:
    user_id             — Cognito sub of the user whose messages are being anonymized
...
endpoints).  Consistent with write_audit_log, deactivate_chat_rooms, and
push_fanout.  AWSLambdaBasicExecutionRole is sufficient — no ENI attachment.
...
# Sentinel value written to sender_id on anonymization.
_ANON_SENDER_ID: str = "[deleted-user]"
...
def handler(event: dict, context: Any) -> dict:
    ...
    if not room_ids:
        logger.info("anonymize_chat_messages_noop", extra={"user_id": user_id, "reason": "empty_room_ids"})
        return {
            "has_more": False,
            "room_ids": room_ids,
    ...
    while idx < len(room_ids):
        ...
        lek = _anonymize_room_messages(user_id, room_id, lek)
        # lek is None once the room is exhausted — advance to next room
        if lek is None:
            idx += 1

    logger.info(
        "anonymize_chat_messages_complete",
...
# ---------------------------------------------------------------------------
# Per-room anonymization
# ---------------------------------------------------------------------------


def _anonymize_room_messages(
    user_id: str,
    room_id: str,
    ...
    then UpdateItem each matching row to set sender_id='[deleted-user]'.

    Handles a single page of Query results.  Returns the LastEvaluatedKey from
    the page (i.e., the DynamoDB pagination cursor), or None when the room is
    exhausted.

    This function is called once per Query page.  The outer loop in handler()
    keeps calling it with the returned key until None is returned, at which
    point it advances to the next room.
    """
    client = _get_dynamo()
    ...
    for item in response.get("Items", []):
        _update_sender_id(item)

    return response.get("LastEvaluatedKey")


def _update_sender_id(item: dict) -> None:
    """
    UpdateItem on a single ChatMessages row to replace sender_id with '[deleted-user]'.
    Only sender_id is modified — all other attributes, including content, are preserved.

    The UpdateExpression uses SET sender_id = :anon_id.
    No ConditionExpression is needed — the operation is idempotent:
    updating sender_id to '[deleted-user]' when it is already '[deleted-user]'

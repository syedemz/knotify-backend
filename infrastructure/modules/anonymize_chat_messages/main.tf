...
# ---------------------------------------------------------------------------
# anonymize_chat_messages Lambda function — story 9.6
#
# Rewrites sender_id to '[deleted-user]' on every ChatMessages row the deleted
# user sent, across all rooms they were a member of.  Called from the soft-delete
# branch of the account-deletion Step Functions state machine inside
# ParallelCleanup (after DeactivateChatRooms has already removed the user's
# ChatRoomMembership rows).
#

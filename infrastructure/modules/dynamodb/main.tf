...
# ---------------------------------------------------------------------------
# account_deletion_audit table — story 9.8
#
# PK: user_id (S) — the Cognito sub of the account being deleted.
# SK: event_id (S) — UUID generated by the WriteAuditLog Lambda at write time.
#   Using a random UUID (not a timestamp-prefix) so that concurrent runs
#   initiated for the same user_id cannot collide on the SK.
#
# TTL: expire_at (N) — Unix epoch seconds. The WriteAuditLog Lambda computes
#   expire_at = int(time.time()) + 7*365*86400 per row. TTL attribute type
#   is Number (enforced by the Lambda writer; DynamoDB TTL requires a numeric
#   attribute but does not declare the type in the table definition itself).
#
# No GSIs: audit queries are always by user_id (PK prefix scan) or by
#   (user_id, event_id) point lookup. No fan-out query pattern exists.
#
# No DynamoDB Stream: audit rows are write-once; no downstream consumer
#   needs real-time event delivery from this table.
#
# billing_mode: PAY_PER_REQUEST — write volume is bounded by deletion events
#   (one per user deletion initiation/completion/failure); on-demand pricing
#   avoids over-provisioning.
#
# deletion_protection_enabled: variable-driven (true in prod, false in dev)
#   — same safety pattern as all other tables in this module.
# ---------------------------------------------------------------------------

resource "aws_dynamodb_table" "account_deletion_audit" {
  name         = "account_deletion_audit"
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "user_id"
  range_key    = "event_id"
